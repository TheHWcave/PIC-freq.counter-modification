...
; | 100Hz     |   00.100   |  06000    |  Khz-dot is flashing, no dot for RPM
; | 255Hz     |   00.255   |  15301    |  Khz-dot is flashing, no dot for RPM
; | 256Hz     |   00.256   | 00.256    | >15301 RPM display switches to Khz     
; | 1.000KHz  |   01.000   | 01.000    |    
; | 10.00KHz  |   10.000   | 10.000    |
; | 100.0KHz  |   100.00   | 100.00    |
; | 1.000MHz  |   1.0000   | 1.0000    |  Note: MHz-dot is steady (not blinking)
; | 10.00MHz  |   10.000   | 10.000    |
; --------------------------
...
period_old_hi  equ  0x3A      ; contains the previously measured period at pcnt-1 (high byte)
period_old_lo  equ  0x3B      ; ... low byte
...
pmodeflag       equ 0x5D      ; see below:
#define PMODE_ON pmodeflag,0       ; 0 = off, 1 = on
#define PMODE_RPM pmodeflag,1      ; 0 = freq, 1 = RPM 
       
ONESECOND       equ .50000	       ; 1 second in 20 us units
PERIOD2FREQ	equ ONESECOND * .1000  ; convert periods into HZ with 2 decimal points
PERIOD2RPM	equ ONESECOND * .600   ; convert periods to RPM
;**************************************************************************
;                                                                         *
; Macros (1)                                                              *
;                                                                         *
...
count_pulses:
	  ...
	  clrf period_old_hi		; clear previous period (at pcnt-1) 
	  clrf period_old_lo            ; 
...
; Here still in 'count_pulses'.
	...
	; 
	;               Note1 that we can only start measuring after the frequency counter value has reached
	;               1 because the time from 0 to 1 is undetermined (the external signal is asynchronous)
	;               This means we need at least 2 Hz for this to produce period measurements.
	;
	;		Note2: this method creates a pcnt that can get too high for the subsequent 32-bit maths 
	;		Therefore later (see P_Conv) we check pcnt and if it is too high, we half it (and 
	;               and the accumilated period of course). 
	;		But if pcnt is odd, halving would introduce a serious math error. Therefore we need to keep
	;		a copy of the previous period at pcnt-1 in period_old. If pcnt is odd, then pcnt-1 must be
	;               even and we use that and period_old instead for the halving operation, and all is well..
	;		
...
P_Measure:
	  movlw .4			; [54] prepare to add 4x4 =16 wasted instructions later 74+16=90
	  movwf period_waste		; [55] 
	  ;  save the previously calculated period (we may need it later)
	  ;
	  movfw period_lo		; [56]
	  movwf period_old_lo		; [57]
	  movfw period_hi		; [58]
	  movwf period_old_hi		; [59]
	  ...
	  movfw pstart_lo 		; [62]
	  movwf period_lo		; [63]
	  bsf STATUS, C			; [64] ; 16-bit subtraction with carry
	  movfw gatecnt_lo		; [65]
	  btfss STATUS, C		; [66]
	  incfsz gatecnt_lo,w		; [67]
	  subwf period_lo,f		; [68]
	  movfw gatecnt_hi		; [69]
	  btfss STATUS, C		; [70]
	  incfsz gatecnt_hi,w		; [71]
	  subwf period_hi,f		; [72]
	  ;
	  incf pcnt,f			; [73]
	  

	  
P_done:	  movf period_waste,w           ; [50, 62, 74  ]
...
;    rounding to the final display resolution
    ...
    ; Display routine for frequencies up to "99.99 MHz" (theoretical):
	  ...
	  ;            digits 0..7 contain the desired value*10. This allows us
	  ;            to round the result if the lowest digit is >5 
displ_100K:
	  btfss PMODE_ON   
	  goto displ_kHz    ; go to normal display if not in period measurement mode
	  
	  movlw .5          ; check if the lowest digit is >5 
	  subwf digit_7,w
	  btfss STATUS, C
	  goto  digit_drop  ; no, we can simply drop it (i.e. round down)
	  movlw digit_6     ; yes, round the result up
	  movwf FSR
displ_rnd:
	  incf  INDF        ; increment the next higher digit
	  movlw .10         ; has it overflowed 
	  subwf INDF,w
	  btfss STATUS, C
	  goto  digit_drop  ; no, we are done rounding
	  clrf  INDF        ; yes: set it to zero and repeat increment for next
	  decf	FSR,f       ; .. higher digit 
	  movlw digit_2
	  subwf FSR,w
	  btfsc STATUS, C   ; until we reach the last possible digit 
	  goto  displ_rnd  
digit_drop:
	; This crude routine moves the result one digit down, loosing the least 
	; significant digit. This is a division by 10. 
	; 
	  movlw digit_3     ; restore the FSR expected by the display
	  movwf FSR         ; 
	  movfw digit_6     ; digit 6 --> digit 7
	  movwf digit_7 
	  movfw digit_5     ; digit 5 --> digit 6
	  movwf digit_6
	  movfw digit_4     ; digit 4 --> digit 5
	  movwf digit_5
	  movfw digit_3     ; digit 3 --> digit 4
	  movwf digit_4
	  movfw digit_2     ; digit 2 --> digit 3
	  movwf digit_3
	  movfw digit_1     ; digit 1 --> digit 2  (should be always zero)
	  movwf digit_2
	  btfsc PMODE_RPM
	  goto display     ; no decimal to indicate RMM 
...
MainRestart:  ; Here we "restart" the counter after exiting from programming mode :
	  ...
	  goto  MainLoop        ; not pressed: go with the default...
	  xorlw 0xff            ; pressed: toggle the setting..
	  andlw 0x02
	  movwf pmodeflag
	  movwf options
...
NoAdjust:  
	  ...
	  ;             We only accept frequencies <= 100Hz in frequency mode, 
	  ;             or < 256 Hz in RPM mode
	  movf  freq_hi,w
	  iorwf freq_mh,w
	  iorwf freq_ml,w
	  btfss STATUS, Z
	  goto  f_positive  ; frequencies > 256 Hz go straight to normal display
	  ;
	  ;  frequency  is < 256 Hz. 
	  ;
	  btfsc PMODE_RPM
	  goto  P_Zero	    ; frequencies < 256 Hz convert to RPM
	  movlw .101	    ; max range for freq <= 100 Hz
	  subwf freq_lo,w
	  btfsc STATUS, C
	  goto  f_positive   ; normal display for freq > 100 Hz
	  ;
	  ;  
...
P_Conv:	  movlw .70
	  ...
	  btfss STATUS, C
	  goto  P_ConvStart
	  ;  TheHWcave: pcnt >= 85 (freq) or 143 (period) overflow the conversion. 
	  ;  we need to half pcnt and period to stay safe but we can't do this 
	  ;  with an odd pcnt or we'll skew the result. If pcnt is odd, we need
	  ;  to revert to pcnt-1 and period_old instead of period
	  ;
	  btfss pcnt,0
	  goto  P_ConvEven
	  ; odd pcnt: need to revert to pcnt-1 and period_old
	  decf  pcnt,f
	  movfw period_old_hi
	  movwf period_hi
	  movfw period_old_lo
...
P_ConvEven:
	  bcf   STATUS, C
	  rrf   pcnt,f      ; half pcnt ...
	  bcf   STATUS, C
	  rrf   period_hi,f ; ..and period 
	  rrf   period_lo,f
P_ConvStart:  
	  ...
	  movwf freq2_lo
	  goto P_Mul
P_RPM:	  movlw (PERIOD2RPM >> .24) & 0xff  ; load the conversion for RPM
	  movwf freq2_hi
	  movlw (PERIOD2RPM >> .16) & 0xff
	  movwf freq2_mh
	  movlw (PERIOD2RPM >> .8) & 0xff
	  movwf freq2_ml
	  movlw PERIOD2RPM & 0xff
	  movwf freq2_lo
...
P_DivEnd: clrf	freq_hi  ; copy the result back into the freq_xx variable
	  movfw pdiv_mh  ; but it still has 10x the desired display resolution
          movwf freq_mh  ; we take care of this later in the display routine
	  movfw pdiv_ml
          movwf freq_ml
	  movfw pdiv_lo
          movwf freq_lo
