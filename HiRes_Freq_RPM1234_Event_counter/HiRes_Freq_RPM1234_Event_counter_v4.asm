;**************************************************************************
; FILE:      HiRes_Freq_RPM1234_Event_counter_v2.asm                      *
...
;                                                                         *
; 2021-03-04 - TheHWcave: Further improvement on the autoranging          *
;	       1: <= 200 Hz   prescaler: 1 period  resolution 0.01Hz      *
;	       1: <= 105.6kHz prescaler: 1 gate 1s resolution    1Hz      *
;	       2: <= 983.0kHz prescaler: 2 gate 1s resolution    2Hz      *
;	       3: <= 3.932MHz prescaler: 2 gate 1/2s resolution  4Hz      *
;	       4: <= 7.864MHz prescaler: 4 gate 1/2s resolution  8Hz      *
;	       5: <=15.728MHz prescaler: 8 gate 1/2s resolution 16Hz      *
;	       6: <=31.457MHz prescaler:16 gate 1/2s resolution 32Hz      *
;              7: else        prescaler:32 gate 1/2s resolution 64Hz      *
;	       with that change the counter now exceeds 100MHz (my version*
;              went up to 112 MHz requiring a change to add one more BCD  *
;              digit and a modification in the display routine            *
;                                                                         *
; 2021-03-02 - TheHWcave: Further improvement on the autoranging          *
;	       1: <= 200 Hz   prescaler: 1 period  resolution 0.01Hz      *
;	       1: <= 105.6kHz prescaler: 1 gate 1s resolution    1Hz      *
;	       2: <= 983.0kHz prescaler: 2 gate 1s resolution    2Hz NEW  *
;	       3: <= 3.932MHz prescaler: 2 gate 1/2s resolution  4Hz NEW  *
;	       4: <= 7.864MHz prescaler: 2 gate 1/4s resolution  8Hz      *
;	       5: <=15.728MHz prescaler: 4 gate 1/4s resolution 16Hz      *
;	       6: <=31.457MHz prescaler: 8 gate 1/4s resolution 32Hz      *
;              7: else        prescaler:16 gate 1/4s resolution 64Hz      *
; 2021-02-20 - TheHWcave: v2 shortend range 3 to avoid 120ns problem      *
;              also fixed an error in the count_pulses loop (one          *
;	       missing NOP (instruction #65) caused the loop to run a	  *
;	       tiny bit too fast and an undereading of 200Hz at 4 MHz     *
;              Note: this error would have been compensated-out when      *
;              when calibrating the oscillator                            *
...
psave_timer    equ  0x53      ; timer for power-save mode (incremented every 0.25 seconds)
psave_freq_lo  equ  0x54      ; low-byte of frequency to detect changes for power-save mode
psave_flags    equ  0x55      ; power-saving flags with the following bits:
#define PSFLAG_ACTIVE psave_flags,0 ; clear:normal mode,  set:power-saving in action (display blanked)

options        equ  0x56      ; display options with the following flag-bits:
#define OPT_PWRSAVE options,0 ; clear:normal mode,  set:power-saving mode enabled

	
period_waste	equ 0x57      ; stores the number of cycle*4 to waste to make up the correct total 
			      ; ... number of instructions in the count_pulses loop
pcnt		equ 0x58      ; number of periods measured 
period_hi	equ 0x59      ; accumulated period in 20us increments (high byte)
period_lo	equ 0x5A      ; ... low byte
pdiv_mh		equ 0x5B      ; used to store the final division result (bits 16..23)
pdiv_ml		equ 0x5C      ; ... bits 8..15
pdiv_lo		equ 0x5D      ; ... bits 0..7
pmodeflag       equ 0x5E      ; 7 6 5 4 3 2 1 0
			      ...
			      ; 0 - - - - 1 0 1    = period measuring off (counter)
...
count_pulses:
	  ...
	  sublw .4
	  btfsc STATUS,C
	  goto count0			; not in counter mode, proceed as normal
	  ;
...
; 2021-03-04 TheHWcave: allow frequencies > 100 MHz
;--------------------------------------------------------------------------

display_freq:
; Display the decimal digits according to the following rules
; 012345678
; 000000Abc => 00A.bc    001.01 to 009.99 Hz       Hz =  decpoint at digit 6 
; 00000ABcd => 0AB.cd    010.01 to 099.99 Hz       Hz =  decpoint at digit 6  
; 000000ABC => 00.ABC    00.100 to 00.999 KHz      kHz = decpoint at digit 5
; 00000ABCD => 0A.BCD    01.000 to 09.999 KHz      kHz = decpoint at digit 5
; 0000ABCDE => AB.CDE    10.000 to 99.999 KHz      kHz = decpoint at digit 5
; 000ABCDEF => ABC.DE    100.00 to 999.99 KHz   100KHz = decpoint at digit 5
; 00ABCDEFG => A.BCDE    1.0000 to 9.9999 Mhz     1MHz = decpoint at digit 2
; 0ABCDEFGH => AB.CDE    10.000 to 99.999 Mhz    10MHz = decpoint at digit 2
; ABCDEFGHI => ABC.DE    100.00 to 1xx.xx Mhz   100MHz = decpoint at digit 2
...
;    rounding to the final display resolution
    ...
    ; Display routine for frequencies up to "199.99 MHz" (theoretical):
          ...
          bnz   displ_MHz               ; 100-MHz-digit non-zero, show frequency in MHz
...
displ_Hz:   ; insert a STEADY POINT at digit 5 to indicate Hz + fractions
	  bsf   digit_6, 7  ; set the decimal point indicating the frequency in Hz .
	  goto	display
displ_kHz:  ; insert a BLINKING POINT to indicate the kilohertz-digit
          ...
          bsf digit_5, 7    ; set the decimal point indicating the frequency in kHz .
          goto  display

displ_MHz:   ; insert a STEADY POINT to indicate the megahertz-digit
          bsf   digit_2, 7  ; set the decimal point indicating the frequency in MHz .
...
RANGE_DET_LOOPS equ  CLOCK/(.30*CYCLES) ; number of gate-time loops to detect the MEASURING RANGE
	   ...
	   ; reworked 02-03-2021 TheHWcave to fix the problem of pulse width dropping below 120ns
	   ;                               and cleanup the code & comments a little
	   ;
           ; Rng  testcount    f_in            prescaler gate_time   resolution
           ; (1)     0..6         0..   13.439 kHz   1       1   second  1Hz (<200 Hz /1/100 Hz)
           ; (1)     7..54         ..  105.599 kHz   1       1   second  1Hz   
	   ; (2)    55..511        ..  983.039 kHz   2       1   second  2Hz 
	   ; (3)   512..2047       .. 3.932159 MHz   2       1/2 second  4Hz
	   ; (4)  2048..4095       .. 7.864319 MHz   2       1/4 second  8Hz
           ; (5)  4096..8191      ...15.728639 MHz   4       1/4 second 16Hz
           ; (6)  8192..16383     ...31.457279 MHz   8       1/4 second 32Hz
           ; (7) 16384..33330     ...63.9      MHz  16       1/4 second 64Hz  
          ...
          goto  Range3
          btfsc freq_ml,0      ; bit 8 set (>=256) -> no Z flag  -> range 2
          goto  Range2
          ...
          goto  Range2         ; freq > 100kHz -> also range 2
		               ; otherwise range 1 (lowest frequencies)
...
Range2:  ; Range 2: prescaler divide by 2, gate time = 1 sec) :
	  ...
	  ; Load the GATE TIMER (as count of loops) for this measuring range.
          ...
          movlw (GATE_TIME_LOOPS)&0ffh ; low byte for 1 second gate time
          movwf gatecnt_lo
	  movlw 1   ; multiply by 2 (=2^1) later to compensate prescaling (1:2) * gate time (1 s)
	  goto  GoMeasure   
	  
Range3:  ; Range 3: prescaler divide by 2 , gate time = 1/2 second gate time :
          movlw PSC_DIV_BY_2            ; let the prescaler divide by 2 while MEASURING...
          call  SetPrescaler            ; safely write <W> into option register
	  incf  psave_timer, f           ; increment power-save timer one more time (0.5 sec-gate instead o
          ; Load the GATE TIMER (as count of loops) for this measuring range.
...
Range4:   ; Range 4: prescaler divide by 4 , gate time = 1/2 second gate time :
...
Range5:   ; Range 5: prescaler divide by 8 , gate time = 1/2 second gate time :
...
Range6:   ; Range 6: prescaler divide by 16 , gate time = 1/2 second gate time :
...
Range7:   ; Range 7: prescaler divide by 32 , gate time = 1/2 second gate time :
...
;
; TheHWcave:check for counter mode
;

	  movfw pmodeflag		; get current mode
	  andlw 0x07			; just the user selection
	  sublw .4
	  BC	 GoMeasure1             ; not in counter mode
MainLoopCnt: 
...
RefreshDisplay:
	...
	; TheHWCave: Load the display with the current mode (freq / rpm / counter)
...
show_mode: 
	  ...
	  sublw .4
	  BNC	sm_count		; if >= 5 : counter mode
...
sm_freq:  movlw CHAR_F
	  ...
	  call conv_char4
	  retlw 0
sm_count: movlw CHAR_c
	  call conv_char0		; set to "couNt"
	  movlw CHAR_o
	  call conv_char1
	  movlw CHAR_u
	  call conv_char2
